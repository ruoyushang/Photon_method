...
void GetBaseLineEvents(string sampleID, string outputName, string pathToNtuples, string ch, bool isData) {
	...
	Float_t MTWino_W01= 0;
	TBranch *b_MTWino_W01 = BaselineTree->Branch("MTWino_W01",&MTWino_W01,"MTWino_W01/Float_t");
	Float_t MTWino_Wmin= 0;
	TBranch *b_MTWino_Wmin = BaselineTree->Branch("MTWino_Wmin",&MTWino_Wmin,"MTWino_Wmin/Float_t");
	...
	for (Long64_t i=0;i<nentries;i+=event_interval) {
		...
		if (jet_n==2) {
			...
			MTWino_W01 = ComputeMT2(z_4vec, dijet_4vec, met_4vec, 0, 0).Compute();
			MTWino_Wmin = -1e10;
		}
		else if (jet_n>=3) {
			jet0_4vec.SetPtEtaPhiM(jet_pT->at(0),jet_eta->at(0),jet_phi->at(0),jet_m->at(0));
			...
			dijet_4vec = jet0_4vec + jet1_4vec;
			MTWino_Wmin = ComputeMT2(z_4vec, dijet_4vec, met_4vec, 0, 0).Compute();
		}
		else if (jet_n==1){
			jet0_4vec.SetPtEtaPhiM(jet_pT->at(0),jet_eta->at(0),jet_phi->at(0),jet_m->at(0));
			MTWino_W01 = ComputeMT2(z_4vec, jet0_4vec, met_4vec, 0, 0).Compute();
			MTWino_Wmin = -1e10;
		}
